...
为了理清这段设计，笔者在本地搭建并跑通了一套 JSON 隔离架构，以替代不加约束的原始状态。我的核心思路是：**默认把所有的 Agent 丢进属于自己独立工作区的 Docker 沙箱里随意折腾，但通过授信内网的 `webchat` 渠道为网关开启一条逃逸提权的特权通道**，从而实现“通过沙箱做日常的破坏性跑通测试，用提权后门完成受信任环境的真机下发”。
